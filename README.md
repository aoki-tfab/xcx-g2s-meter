...
| ブロック | 説明 | 入力コネクタ | 範囲 |
| --- | --- | --- | --- |
| `電圧の値(V)` | 計測した電圧を返す | アナログ **A1** | 0 〜 20 V |
| `電流の値(A)` | 計測した電流を返す | アナログ **A2** | 0 〜 2 A |
| `電力の値(W)` | 電圧 × 電流 を返す | A1・A2 から算出 | 0 〜 40 W |

いずれも入力欄のない、値を返すだけのレポーターブロックです（「環境I2Cの温度(°C)」などと同じ形）。ボード未接続のときは空文字を返します。
